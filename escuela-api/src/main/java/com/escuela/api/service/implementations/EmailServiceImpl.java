...
public class EmailServiceImpl implements EmailService {
         ...
         Mail mail = new Mail();
            ...
            try {
                email.sendInDividualEmail(mail.getEmail(), mail.getSubject(),
                        templateService.getData(model, "/email_template/contactus_mail.ftl"));
    ...
    @Deprecated
    public void sendRecruiterProfileVerificationEmail(Mail mValue){
        Mail mail = new Mail();
        mail.setEmail(mValue.getEmail());
        mail.setMessage(mValue.getMessage());
        mail.setSubject("[Esquela-Tech] Customer Enquiry Received ");
        mail.setFrom("ESQUELA-TECH");
        mail.setAck("Thanks for registering with us, " +"Please verify the registered email.");
        Map<String, Object> model = new HashMap<String,Object>();
        model.put("message",mail);
        model.put("greeting", "Hello "+mValue.getLastName() +" "+mValue.getFirstName()+",");
        model.put("baseUrl", "www.escuelatech.com"); //not used
        try {
            email.sendInDividualEmail(mail.getEmail(), mail.getSubject(),templateService.getData(model, "/contactus_mail.ftl"));
            System.err.println(String.format("Contact Email sent from: %s", mail.getFrom()));
        } catch (Exception e) {
            e.printStackTrace(); //todo hANDLE eXCEPTION
        }
